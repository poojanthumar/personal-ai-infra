...
|---|---|---|---|---|
| 1 | Local models on the Mac | [01](02-local-ai.md) | 1 h | Everything else |
| 2 | **Photo/video ranker** | [07](03-photo-ranker.md) | 3 h | 🏆 Real value on day one, ₹0 |
| 3 | LiteLLM router | [02](06-litellm.md) | 1 h | Budgets, privacy gate |
| 4 | Oracle box + queue API | [03](04-oracle-box.md) | 3 h | Always-on |
| 5 | Mac agent | [06](05-mac-agent.md) | 2 h | Remote requests reach your files |
| 6 | OpenClaw on WhatsApp | [15](07-openclaw.md) | 3 h | The front door |
| 7 | Reels pipeline | [08](09-reels.md) | 3 h | Videos from ranked clips |
| 8 | Photo albums | [10](10-albums.md) | 3 h | Event grouping |
| 9 | Web research | [12](11-web-research.md) | 2 h | Search and read, ₹0 |
| 10 | Dashboard | [11](12-dashboard.md) | 2 h | See spend and jobs |
...
| # | Do | Where | File |
|---|---|---|---|
| 1 | Install ffmpeg, exiftool, torch, open_clip, imagehash, scenedetect | Mac | [01](02-local-ai.md) §8 |
| 2 | Confirm `torch.backends.mps.is_available()` is `True` | Mac | [01](02-local-ai.md) §8 |
| 3 | Download `ava+logos-l14-linearMSE.pth` to `~/.cache/aesthetic/` | Mac | [07](03-photo-ranker.md) §2 |
| 4 | Create `media_db.py` — the SQLite schema | Mac | [07](03-photo-ranker.md) §3 |
| 5 | Create `rank_media.py` | Mac | [07](03-photo-ranker.md) §4 |
| 6 | Run on a 20-file test folder | Mac | [07](03-photo-ranker.md) §5 |
| 7 | Run on the real folder with `caffeinate -s` | Mac | [07](03-photo-ranker.md) §5 |
| 8 | **Rate ~100 photos, fit weights to your taste** | Mac | [07](03-photo-ranker.md) §6 |
| 9 | Caption the top 500 using `private-local` | Mac | [07](03-photo-ranker.md) §7 |
| 10 | Register `rank_media` as a job handler | Mac | [06](05-mac-agent.md) §8 |
| 11 | Define the `rank_media` tool in OpenClaw | Oracle | [15](07-openclaw.md) §3 |
...
**Re-run the safety test in file 05 §4 after this change.** Every line must
print `OK`.
...
| # | Do | Where | File |
|---|---|---|---|
| 1 | `pip install librosa soundfile` | Mac | [08](09-reels.md) §1 |
| 2 | Create `beats.py`, test on a track | Mac | [08](09-reels.md) §1 |
| 3 | Put music **you own** in `~/Media/Music` | Mac | [08](09-reels.md) ⚠️ |
| 4 | Create `manifest.py` | Mac | [08](09-reels.md) §2 |
| 5 | Create `plan_reel.py` **with the validator** | Mac | [08](09-reels.md) §3 |
| 6 | Create `build_reel.py` | Mac | [08](09-reels.md) §4 |
| 7 | Render one by hand end to end | Mac | [08](09-reels.md) §5 |
| 8 | Register `reel_render` handler | Mac | [06](05-mac-agent.md) §8 |
| 9 | Define `make_reel` tool | Oracle | [15](07-openclaw.md) §3 |
| 10 | Add beat-snapping in code | Mac | [08](09-reels.md) §7 |
...
| # | Do | Where | File |
|---|---|---|---|
| 1 | Choose **Immich** (own server) or **osxphotos** (Apple Photos) | Mac | [10](10-albums.md) |
| 2 | If Immich: install via their official compose file | Mac or Oracle | [10](10-albums.md) §1 |
| 3 | If Immich: comment out `immich-machine-learning` to save memory | — | [10](10-albums.md) §2 |
| 4 | If osxphotos: `pip install osxphotos`, import metadata | Mac | [10](10-albums.md) Option B |
| 5 | Create `group_photos.py` — time and distance gaps | Mac | [10](10-albums.md) §5 |
| 6 | Run it, then **tune `TIME_GAP_HOURS`** to match real trips | Mac | [10](10-albums.md) §5 |
| 7 | Create `name_events.py` | Mac | [10](10-albums.md) §6 |
| 8 | Register `group_events` handler | Mac | [06](05-mac-agent.md) §8 |
...
| Generic names | Improve the captioning prompt in file 03 §7 |
...
| # | Do | Where | File |
|---|---|---|---|
| 1 | Run SearXNG in Docker with `formats: json` | Oracle | [12](11-web-research.md) §1 |
| 2 | Confirm `?format=json` returns JSON, not 403 | Oracle | [12](11-web-research.md) §2 |
| 3 | `pip install crawl4ai`, run `crawl4ai-setup` | Oracle | [12](11-web-research.md) §4 |
| 4 | Create `web_search.py` and `web_read.py` | Oracle | [12](11-web-research.md) §3, §5 |
| 5 | Wire both as OpenClaw tools | Oracle | [15](07-openclaw.md) §3 |
| 6 | Optional: attach the `fetch` MCP server instead | Oracle | [15](07-openclaw.md) §5 |
...
Use the prompts in [file 14](14-verification-prompts.md).
...
3. Add it as `tier3-smart` in LiteLLM ([file 06](06-litellm.md) §3).
...
2. Add it to OpenClaw's MCP config on Oracle — stdio command, or URL plus
   credentials ([file 07](07-openclaw.md) §5).
...
| ⚠️ Scope servers per profile | **Not documented for OpenClaw.** Check first — prompt in file 07 |
...
**Nothing in this file needs verification** — it describes how the pieces you
built interact, not facts about the world.

Every ⚠️ it references is verified in the file that owns it:

| Referenced claim | Verified in |
|---|---|
| Token and rupee costs per route | [01](01-decisions-and-costs.md) |
| Oracle capacity and limits | [04](04-oracle-box.md) |
| OpenClaw's tool interface | [07](07-openclaw.md) — largely unverified |
| MCP token cost and catalogue | [08](08-mcp-servers.md) |
| Google Photos closure | [10](10-albums.md) |

**One thing you should verify yourself, by running it:** the actual wall-clock
time and token count of each flow, from your own dashboard ([12](12-dashboard.md))
after a week. My estimates are arithmetic, not measurements.
